...
func (px *Paxos) alterMaxPrepare(seq int) {
  go func(px *Paxos) {
    time.Sleep(500 * time.Millisecond)	 
	px.mu.Lock()
	if seq >= px.maxPrepare {
      px.maxPrepare = seq
	}
	px.mu.Unlock()
  }(px)
...
func (px *Paxos) alterMaxAccept(seq int, v interface{}) {
  go func(px *Paxos) {
    time.Sleep(500 * time.Millisecond)
	px.mu.Lock()
	if seq > px.maxPrepare {
...
func (px *Paxos) Accept(args *AcceptArgs, reply *AcceptReply) error {
  ...
  if args.N < px.maxPrepare {
    ...
    if args.N > px.maxAccept {
      px.alterMaxAccept(args.N, args.Value)
	}
	reply.N = px.maxAccept
	reply.Success = true
	return nil
